...
test("compares semantic versions and blocks downgrade", () => { assert.equal(compareVersions("1.2.0", "1.1.9"), 1); assert.equal(compareVersions("0.1.10", "0.1.9"),1); assert.equal(decideUpdate({currentVersion:"2.0.0",manifest:manifest()}).reason,"downgrade-blocked"); });
