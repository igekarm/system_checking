...
test("checks, downloads and verifies a signed update",async()=>{
  ...
  let downloadedUrl; const client={latestRelease:async()=>({manifest,release:{body:"notes",assets:[{name:"DB-Tools.exe",browser_download_url:"public-installer"}]}}),downloadAsset:async(url)=>{downloadedUrl=url;return new Response(installer)}};
