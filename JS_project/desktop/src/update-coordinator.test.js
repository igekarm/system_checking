...
test("checks, downloads and verifies a signed update",async()=>{
  ...
  const client={latestRelease:async()=>({manifest,release:{body:"notes",assets:[{name:"DB-Tools.exe",url:"asset-api"}]}}),downloadAsset:async()=>new Response(installer)};
  ...
  assert.equal((await coordinator.check()).state,"available"); const ready=await coordinator.download(); assert.equal(ready.state,"ready"); assert.deepEqual(await readFile(ready.installerPath),installer);
});
