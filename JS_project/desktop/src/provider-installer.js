...
export const ORACLE_PROVIDER=Object.freeze({
  id:"oracle",name:"Oracle Database",version:"7.0.1",mode:"Thin / Thick",minimumDatabaseVersion:"11.2 (Thick) / 12.1 (Thin)",
...
export function normalizeOracleClientDir(value){
  const directory=String(value??"").trim();
  if(!directory)return "";
  if(!path.isAbsolute(directory))throw new Error("Каталог Oracle Instant Client должен быть указан абсолютным путём");
...
export class ProviderInstaller {
  ...
  async loadOracleDriver(oracleClientDir=""){
    const clientDir=normalizeOracleClientDir(oracleClientDir);
    const requestedMode=clientDir?"Thick":"Thin";
    if(this.driver){
      if(this.oracleMode!==requestedMode||this.oracleClientDir!==clientDir)throw new Error("Режим Oracle уже инициализирован с другими настройками. Перезапустите приложение после изменения каталога Instant Client");
      return this.driver;
    }
    if(!await this.isOracleInstalled())throw new Error("Oracle provider не установлен. Включите Oracle в настройках приложения");
    const driver=require(path.join(this.oracleDirectory(),"package"));
    if(clientDir){
      if(process.platform==="win32")await access(path.join(clientDir,"oci.dll")).catch(()=>{throw new Error(`В каталоге Oracle Instant Client не найден oci.dll: ${clientDir}`)});
      try{driver.initOracleClient(process.platform==="win32"?{libDir:clientDir}:{});}catch(error){throw new Error(`Не удалось запустить Oracle Thick mode: ${error.message}`,{cause:error});}
    }
    this.driver=driver;this.oracleMode=requestedMode;this.oracleClientDir=clientDir;return driver;
  }
}
